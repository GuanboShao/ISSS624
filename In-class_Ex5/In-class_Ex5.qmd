...
```{r}
pacman::p_load(sf, spdep, tidyverse, ggpubr, tmap, corrplot, GWmodel, funModeling, blorr, skimr, caret)
```

## Importing Data into R environment

The code chunk below uses `read_rds()` of **Base R** to import rds file *Osun* and *Osun_wp_sf* into R.
...
The code chunk below uses `freq()` to check the distribution of *status* field.
...
The code chunk below uses `filter_at()` to keep selected variables, remove NA values and `all_vars()` to change *usage_capacity* into numeric values.
...
The code chunk below visualize the strength of correlation between variables.
